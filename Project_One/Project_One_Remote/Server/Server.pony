...
actor Worker
  ...
  new create(boss: Main, start: U64, end_val: U64, seqLength: U64, ultimateEnd: U64, conn: TCPConnection) =>
   ...
   // Run the sum of consecutive squares operation
   be run() =>
  ...
  // Workers need to check in when done
  be checkin() =>
    let end_time: (I64, I64) = Time.now()
    let elapsed_seconds = end_time._1 - _start_time._1 
    let elapsed_nanoseconds = end_time._2 - _start_time._2 
...
actor Main
  ...
  new create(env: Env) =>
    ...
    TCPListener(TCPListenAuth(env.root),
      recover MyTCPListenNotify(env, this) end, "0.0.0.0", "8999")
    env.out.print("Successfully bound to port 8999.")

  // Assign the work to workers
  be assign_work(endValue: U64, seqLength: U64, numberWorkers: U64, conn: TCPConnection) =>
    ...
    let actualWorkers: U64 = numberWorkers
...
class MyTCPConnectionNotify is TCPConnectionNotify
  ...
  fun ref received(
    ...
    try
      let end_val: U64 = parts(0)?.u64()? 
      let seqLength: U64 = parts(1)?.u64()? 
...
// Server will list on local address
